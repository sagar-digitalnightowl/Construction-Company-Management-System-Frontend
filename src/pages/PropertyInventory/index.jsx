...
export default function PropertyInventory() {
  ...
  // ✅ NAYA STATE: Booking search ke liye
  ...
  const handleViewProject = async (id) => {
    ...
    if (project) {
      ...
      fetchProjectAgreements(id);
      fetchSiteEngineers(id);
      // Modal open hone par booking search clean kar dena optional hai
      setBookingSearch(""); // ya aap chahe to ise comment kar sakte hain
    }
  };

  // ✅ UPDATED: Booking pagination ke sath search term bhi bhejo
  const handleBookingPageChange = (page) => {
    if (!selectedProjectId) return;
    fetchProjectBookings(selectedProjectId, {
      page,
      limit: 10,
      search: bookingSearch, // <-- taake search filter page change par bhi bana rahe
    });
  };

  // ✅ NAYA HANDLER: Search button dabane par
  const handleBookingSearch = () => {
    if (!selectedProjectId) return;
    fetchProjectBookings(selectedProjectId, {
      page: 1, // search hamesha page 1 se shuru
      limit: 10,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ UPDATED: Project Detail Dialog with new props */}
      <ProjectDetailModal
        open={detailOpen}
        onOpenChange={setDetailOpen}
        project={selectedProject}
