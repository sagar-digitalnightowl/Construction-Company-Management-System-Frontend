...
//     FileText, Users, AlertCircle, PlayCircle
// } from "lucide-react";
...
//     // ---- State ----
//     // Naya Unified Payroll Dialog State
...
//             fetchMyPayrollBatches(); // Refresh batches list after submission
//         }
//     };
...
//         setPayrollDialogOpen(true); // Open the unified dialog for resubmission
//     };
...
export function SalaryTab() {
    ...
    // Helper: "August" + "2026" → "2026-08"
    const toMonthStr = (monthName, year) => {
        const monthNames = [
            "January", "February", "March", "April", "May", "June",
            "July", "August", "September", "October", "November", "December"
        ];
        const mIndex = monthNames.indexOf(monthName);
        if (mIndex === -1) return null;
        return `${year}-${String(mIndex + 1).padStart(2, '0')}`;
    };

    // Helper: "2026-08" → "August 2026"
    ...
    useEffect(() => {
        // ✅ FIX: Convert month name to "YYYY-MM" for backend
        const formattedMonth = toMonthStr(slipMonth, slipYear);
        if (formattedMonth) {
            fetchAllSalarySlips({
    ...
    const handleResubmit = (batch) => {
        // ✅ FIX: batch.monthName is "August", batch.month is "2026-08"
        if (batch.monthName) {
            setSelectedMonth(batch.monthName);
        }
        setSelectedYear(batch.year ? batch.year.toString() : (batch.month?.split('-')[0] || ''));
    ...
    const handleBulkGenerate = async () => {
        ...
        if (res) {
            setBulkDialogOpen(false);
            setBulkDepartment("all");
            // ✅ FIX: Use formatted month for refresh
            const formattedMonth = toMonthStr(slipMonth, slipYear);
            if (formattedMonth) {
                fetchAllSalarySlips({ month: formattedMonth, page: 1, limit: 50 });
            }
        }
    };
    ...
    // ✅ FIX: Aligned with SalarySlip schema enum values
    const getStatusBadge = (status) => {
        switch (status) {
            case "Pending": return "warning";
            case "Processed": return "default";
                                        ...
                                        <TableBody>
                                            {slips.map((slip) => (
                                                <TableRow key={slip._id}>
                                                    ...
                                                    {/* ✅ FIX: Show "August 2026" instead of raw "2026-08" */}
                                                    ...
                                                    <TableCell>
                                                        {/* ✅ FIX: Uses schema-matching statuses only */}
                                                    <Badge variant={getStatusBadge(slip.paymentStatus)}>
                                                        {slip.paymentStatus || "Pending"}
                                                    </Badge>
                                                    </TableCell>
                                    ...
                                    <TableBody>
                                        {myPayrollBatches.map((batch) => (
                                            <TableRow key={batch._id}>
                                                <TableCell className="font-medium">
                                                    {/* ✅ FIX: Show "August 2026" instead of raw "2026-08" */}
                                                    {batch.monthName || formatMonthDisplay(batch.month)}
                                                </TableCell>
                                                <TableCell>{batch.totalEmployees}</TableCell>
                                                <TableCell>₹{(batch.totalAmount || 0).toLocaleString('en-IN')}</TableCell>
                                                <TableCell>
                                                    {/* ✅ FIX: Separate badge for PayrollBatch statuses */}
                                                    <Badge variant={
                                                        batch.status === "Pending Finance Approval" ? "warning" :
                                                        batch.status === "Approved" ? "default" :
                                                        batch.status === "Sent to Bank" ? "secondary" :
                                                        batch.status === "Bank Processed" ? "success" :
                ...
                <DialogContent className="sm:max-w-[400px]">
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        {selectedSlip && (
                            <div className="text-sm bg-muted p-3 rounded-md">
                                <p><strong>Employee:</strong> {selectedSlip.employeeId?.name || "Unknown"}</p>
                                <p><strong>Month:</strong> {formatMonthDisplay(selectedSlip.month)}</p>
                                <p><strong>Net Pay:</strong> ₹{(selectedSlip.netSalary || 0).toLocaleString('en-IN')}</p>
                            </div>
                        )}
                    </div>
                    <DialogFooter>
