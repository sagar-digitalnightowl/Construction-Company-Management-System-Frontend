...
export default function Bookings() {
	...
	useEffect(() => {
		const loadTowersForFilter = async () => {
			if (projectName === "all") {
				setFilterTowers([]);
				return;
			}

			const selectedProject = filterProjects.find(
			...
			try {
				...
				const res = await projectApi.towers.getAll(selectedProject._id);
				setFilterTowers(res.data?.data?.towers || []);
			} catch (err) {
				setFilterTowers([]);
		...
		loadTowersForFilter();
		// towerName is no longer reset here — handleProjectChange already did it synchronously
	}, [projectName, filterProjects]);
		...
		<div className="space-y-6">
			...
			<div className="flex flex-col xl:flex-row gap-3 justify-between items-start xl:items-center">
				...
				<div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
					...
					<Select
						value={towerName}
						onValueChange={(val) => setTowerName(val)}
						disabled={projectName === "all" || loadingTowers}
					>
						<SelectTrigger className="w-full sm:w-56 bg-background">
							<SelectValue
								placeholder={loadingTowers ? "Loading towers..." : "Select Tower"}
							/>
						</SelectTrigger>
						<SelectContent>
