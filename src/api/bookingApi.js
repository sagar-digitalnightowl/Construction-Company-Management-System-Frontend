...
export const bookingApi = {
	exportBookings: (params) =>
		api.get("/booking/export", {
			params,
			responseType: "blob",
		}),

	exportBooking: (bookingId) =>
		api.get(`/booking/export/${bookingId}`, {
			responseType: "blob",
		}),
	...
	getManagerMyBookings: (params) =>
		api.get("/booking/manager/my-bookings", { params }),

	// Get summary/dashboard stats for logged-in manager (PM/BM/TM)
	getManagerSummary: (params) =>
		api.get("/booking/manager/summary", { params }),

	// ==================== INSTALLMENTS ====================
	// Get installments by booking ID
	getInstallmentsByBooking: (bookingId) =>
		api.get(`/booking/installments/booking/${bookingId}`),
	// Get my installments (client only)
	getMyInstallments: (params) =>
		api.get("/booking/installments/my", { params }),
	// Get installments for a project (admin/PM/director/accountant)
	getProjectInstallments: (projectId, params) =>
		api.get(`/booking/installments/project/${projectId}`, { params }),
	// Get installment summary for a booking
	getInstallmentSummary: (bookingId) =>
		api.get(`/booking/installments/summary/${bookingId}`),
	// Get upcoming installments (client)
	getUpcomingInstallments: (days) =>
		api.get("/booking/installments/upcoming", { params: { days } }),
	// Pay an installment
	payInstallment: (installmentId, data) =>
		api.post(`/booking/installments/${installmentId}/pay`, data),
	// Mark installment as due by milestone (admin/PM)
	markInstallmentDueByMilestone: (data) =>
		api.post("/booking/installments/mark-due", data),
	// Mark overdue installments (admin/director)
	markOverdueInstallments: () =>
		api.post("/booking/installments/mark-overdue"),
	// Create 13 installments from template (admin/PM)
	createInstallmentsFromTemplate: (bookingId, data) =>
		api.post(`/booking/${bookingId}/installments/template`, data),
	// Create custom installments (admin/PM)
	createCustomInstallments: (bookingId, data) =>
		api.post(`/booking/${bookingId}/installments`, data),

	// ==================== AGREEMENT DOCUMENTS ====================
	// Get presigned URL for agreement upload (admin/PM)
	getAgreementPresignedUrl: (bookingId, data) =>
		api.post(`/booking/${bookingId}/agreement/presigned-url`, data),
	// Confirm agreement upload
	confirmAgreementUpload: (bookingId, data) =>
		api.post(`/booking/${bookingId}/agreement/confirm`, data),
	...
	searchCustomer: (searchTerm) =>
		api.get(
			`/booking/customer/search?search=${encodeURIComponent(searchTerm)}`,
		),
};
