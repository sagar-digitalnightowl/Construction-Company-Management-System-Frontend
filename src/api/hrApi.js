...
export const hrApi = {
    ...
    getEmployeesByDepartment: (departmentId, params) => api.get(`/hr/departments/${departmentId}/employees`, { params }),
    getDepartmentsWithEmployeeCount: () => api.get('/hr/departments/with-employee-count'),
    ...
    getTodayAttendanceAnalytics: () => api.get('/hr/attendance/today-analytics'),
    filterAttendance: (data) => api.post('/hr/attendance/filter', data),
    getMonthlyAttendanceReport: (data) => api.post('/hr/attendance/monthly-report', data),
    getEmployeeAttendanceById: (employeeId, params) => api.get(`/hr/attendance/employee/${employeeId}`, { params }),

    // Salary
    generateSalarySlip: (data) => api.post('/hr/salary/generate', data),
    generateBulkSalarySlips: (data) => api.post('/hr/salary/bulk-generate', data),
    getMySalarySlips: () => api.get('/hr/salary/my-slips'),
    getEmployeeAllSalarySlips: (employeeId) => api.get(`/hr/salary/employee/${employeeId}`),
    getSalarySlipById: (id) => api.get(`/hr/salary/${id}`),
    updateSalarySlipStatus: (id, data) => api.put(`/hr/salary/${id}/status`, data),
    ...
    getEmployeeLeaveBalance: (employeeId) => api.get(`/hr/leaves/balance/${employeeId}`),
    ...
    getEmployeeCurrentShift: (employeeId) => api.get(`/hr/shifts/employee/${employeeId}`),
    ...
    markBulkLaborAttendance: (data) => api.post('/hr/labor-attendance/bulk', data),
    getLaborAttendance: (params) => api.get('/hr/labor-attendance', { params }),
    getLaborAttendanceSummary: (laborId, params) => api.get(`/hr/labor-attendance/summary/${laborId}`, { params }),
    getDailyLaborAttendance: (date, params) => api.get(`/hr/labor-attendance/daily/${date}`, { params }),
