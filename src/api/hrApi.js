...
export const hrApi = {
	...
	// ==================== EMPLOYEE DASHBOARD ====================
	getEmployeeDashboard: () => api.get("/hr/employee/dashboard"),
	...
	getAllExpenseCategories: (params) =>
		api.get("/hr/expense-category", { params }),
	getExpenseCategoryById: (id) => api.get(`/hr/expense-category/${id}`),
	updateExpenseCategory: (id, data) =>
		api.put(`/hr/expense-category/${id}`, data),
	...
	refundExpenseTicket: (id, data) =>
		api.put(`/hr/expenses/${id}/refund`, data),

	// ==================== 🏦 FINANCE EXPENSES & REPORTS (NEW) ====================

	// Finance Executive raises payment for a vendor/contractor
	createFinanceExpense: (data) => api.post("/hr/finance-expenses", data),

	// Finance Executive gets their own raised payments
	getFinanceExecutiveDashboard: (params) =>
		api.get("/hr/finance-expenses/my-dashboard", { params }),

	// Expense Summaries (from Finance module)
	getFinanceProjectExpenses: (projectId) =>
		api.get(`/finance/expenses/project/${projectId}`),
	getFinanceEmployeeExpenses: (employeeId, params) =>
	...
	getEmployeeWallet: (employeeId) =>
		api.get(`/hr/wallet/employee/${employeeId}`),
	...
	getPendingPayments: (params) =>
		api.get("/hr/wallet/pending-payments", { params }),
};
