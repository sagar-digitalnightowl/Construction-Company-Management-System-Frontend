...
//     // Fetch bookings list for a project
//     const fetchProjectBookings = useCallback(async (id) => {
//         setLoading(true);
//         try {
//             const res = await propertyInventoryApi.getProjectBookings(id);
...
export const usePropertyInventory = () => {
    ...
    const fetchProjectBookings = useCallback(async (id) => {
        ...
        try {
            const res = await propertyInventoryApi.getProjectBookings(id);
            // ✅ UPDATED: Handle both nested and direct array responses
            const data = res.data?.data || res.data; 
            setProjectBookings(Array.isArray(data) ? data : []);
        } catch (err) {
            toast.error("Failed to load bookings");
        } finally {
            setLoading(false);
    ...
    const fetchProjectAgreements = useCallback(async (id) => {
        ...
        try {
            const res = await propertyInventoryApi.getProjectAgreements(id);
            // ✅ UPDATED: Apply safe array extraction here as well
            const data = res.data?.data || res.data;
            setProjectAgreements(Array.isArray(data) ? data : []);
        } catch (err) {
            toast.error("Failed to load agreements");
        } finally {
            setLoading(false);
