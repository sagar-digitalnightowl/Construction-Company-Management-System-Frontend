...
export const useProject = () => {
	...
	const [milestonePaymentSummary, setMilestonePaymentSummary] =
		useState(null);
	const [milestoneBuyers, setMilestoneBuyers] = useState(null);
	const [milestoneBuyerDetails, setMilestoneBuyerDetails] = useState(null);
	const [milestoneBuyersPagination, setMilestoneBuyersPagination] = useState({
		page: 1,
		limit: 10,
		total: 0,
		pages: 0,
	...
	const fetchProject = useCallback(async (id, showLoading = true) => {
		...
		try {
			...
			setBoq(
				data?.boq || { items: [], totalAmount: 0, isApproved: false },
			);
	...
	// =========================
	/**
	 * Create a new project with cover image and gallery images
	 * @param {Object} data - Project data
	 * @param {string} data.coverImageKey - File key from presigned URL for cover image (optional)
	 * @param {string[]} data.galleryImageKeys - File keys for gallery images (optional)
	 *
	 * @example
	 * // Step 1: Upload cover image using presigned URL
	 * // Step 2: Upload gallery images using presigned URL
	 * // Step 3: Create project with keys
	 * createProject({
	 *   name: "Sky Residency",
	 *   location: "Mumbai",
	 *   startDate: "2024-01-01",
	 *   coverImageKey: "projects/PROJECT_ID/123456789-cover.jpg",
	 *   galleryImageKeys: ["projects/PROJECT_ID/123456789-gallery1.jpg"]
	 * });
	 */
	...
	// =========================
	/**
	 * Update an existing project with cover image and gallery images
	 ...
	 *
	 * @example
	 * updateProject("65a1b2c3d4e5f67890abcdef", {
	 *   name: "Updated Name",
	 *   coverImageKey: "projects/PROJECT_ID/987654321-new-cover.jpg",
	 *   galleryImageKeys: ["projects/PROJECT_ID/987654321-new-gallery1.jpg"]
	 * });
	 */
	const updateProject = useCallback(
		async (id, data) => {
			...
			try {
				...
				throw new Error(
					res.data?.message || "Failed to update project",
				);
			} catch (err) {
				toast.error(
	...
	const addWeatherLog = useCallback(
		async (id, logData) => {
			try {
				...
				toast.error(
					err.response?.data?.message || "Failed to add weather log",
				);
				return false;
			}
		},
	...
	const createChecklist = useCallback(
		async (id, checklistData) => {
			try {
				const res = await projectApi.createSafetyChecklist(
					id,
					checklistData,
				);
				if (res.data?.success) {
	...
	const updateChecklist = useCallback(
		async (id, checklistId, data) => {
			try {
				const res = await projectApi.updateSafetyChecklist(
					id,
					checklistId,
					data,
				);
	...
	const removeTeamMember = useCallback(
		async (projectId, userId) => {
			try {
				const res = await projectApi.removeTeamMember(
					projectId,
					userId,
				);
				if (res.data?.success) {
	...
	const updateTeamRole = useCallback(
		async (projectId, userId, roleData) => {
			try {
				const res = await projectApi.updateTeamRole(
					projectId,
					userId,
					roleData,
				);
	...
	// =========================
	/**
	 * Cancel all bookings for a project
	 * @param {string} id - Project ID
	 * @param {Object} data - Optional cancellation data (reason, etc.)
	 * @returns {Promise<boolean>} - Returns true if successful
	 *
	 * @example
	 * cancelAllBookings("65a1b2c3d4e5f67890abcdef", {
	 *   reason: "Project cancelled",
	 *   cancelledBy: "admin"
	 * });
	 */
	const cancelAllBookings = useCallback(async (id, data = {}) => {
		try {
			const res = await projectApi.cancelAllBookings(id, data);
			if (res.data?.success) {
				toast.success("All bookings cancelled successfully");
				return true;
			}
			throw new Error(res.data?.message || "Failed to cancel bookings");
		} catch (err) {
			toast.error(
				err.response?.data?.message || "Failed to cancel all bookings",
			);
			return false;
		}
	}, []);
	...
	const resolveIssue = useCallback(
		async (projectId, issueId) => {
			try {
				const res = await projectApi.resolveIssue(projectId, issueId, {
					resolved: true,
				});
				if (res.data.success) {
					await fetchProject(projectId, false);
	...
	const addTower = useCallback(
		async (projectId, towerData) => {
			try {
				const res = await projectApi.towers.create(
					projectId,
					towerData,
				);
				if (res.data.success) {
				...
				toast.error(
					err.response?.data?.message || "Failed to add tower",
				);
				return false;
			}
		},
	...
	const updateTower = useCallback(
		async (projectId, towerIndex, towerData) => {
			try {
				const res = await projectApi.towers.update(
					projectId,
					towerIndex,
					towerData,
				);
				...
				toast.error(
					err.response?.data?.message || "Failed to update tower",
				);
				return false;
			}
		},
	...
	const deleteTower = useCallback(
		async (projectId, towerIndex) => {
			try {
				const res = await projectApi.towers.delete(
					projectId,
					towerIndex,
				);
				if (res.data.success) {
				...
				toast.error(
					err.response?.data?.message || "Failed to delete tower",
				);
				return false;
			}
		},
	...
	const addFloor = useCallback(
		async (projectId, towerIndex, floorData) => {
			try {
				const res = await projectApi.towers.addFloor(
					projectId,
					towerIndex,
					floorData,
				);
	...
	const updateFloor = useCallback(
		async (projectId, towerIndex, floorIndex, floorData) => {
			try {
				const res = await projectApi.towers.updateFloor(
					projectId,
					towerIndex,
					floorIndex,
					floorData,
	...
	const deleteFloor = useCallback(
		async (projectId, towerIndex, floorIndex) => {
			try {
				const res = await projectApi.towers.deleteFloor(
					projectId,
					towerIndex,
					floorIndex,
				);
	...
	const addFlat = useCallback(
		async (projectId, towerIndex, floorIndex, flatData) => {
			try {
				const res = await projectApi.towers.addFlat(
					projectId,
					towerIndex,
					floorIndex,
					flatData,
				...
				toast.error(
					err.response?.data?.message || "Failed to add flat",
				);
				return false;
			}
		},
	...
	const updateFlat = useCallback(
		async (projectId, towerIndex, floorIndex, flatIndex, flatData) => {
			try {
				const res = await projectApi.towers.updateFlat(
					projectId,
					towerIndex,
					floorIndex,
					flatIndex,
				...
				toast.error(
					err.response?.data?.message || "Failed to update flat",
				);
				return false;
			}
		},
	...
	const deleteFlat = useCallback(
		async (projectId, towerIndex, floorIndex, flatIndex) => {
			try {
				const res = await projectApi.towers.deleteFlat(
					projectId,
					towerIndex,
					floorIndex,
					flatIndex,
				...
				toast.error(
					err.response?.data?.message || "Failed to delete flat",
				);
				return false;
			}
		},
	...
	const fetchAvailableFlats = useCallback(
		async (projectId, towerName, params) => {
			try {
				const res = await projectApi.towers.getAvailableFlats(
					projectId,
					towerName,
				);
				if (res.data.success) {
	...
	const fetchMilestonePaymentSummary = async (projectId) => {
		try {
			setLoading(true);

			const res = await projectApi.getMilestonePaymentSummary(projectId);

			if (res.data.success) {
				setMilestonePaymentSummary(res.data.data);
				return res.data.data;
			}
		} catch (error) {
			toast.error(
				error?.response?.data?.message ||
					"Failed to fetch milestone payment summary",
			);
		} finally {
			setLoading(false);
		}
	...
	const fetchMilestoneBuyers = useCallback(async (params = {}) => {
		try {
			setLoading(true);

			const res = await projectApi.getMilestoneBuyers(params);

			if (res.data.success) {
				setMilestoneBuyers(res.data.data);

				setMilestoneBuyersPagination(
			...
			toast.error(
				error?.response?.data?.message ||
					"Failed to fetch milestone buyers",
			);
		} finally {
			setLoading(false);
		}
	...
	const fetchMilestoneBuyerDetails = async (bookingId, params = {}) => {
		try {
			setLoading(true);

			const res = await projectApi.getMilestoneBuyerDetails(
				bookingId,
				params,
			);

			if (res.data.success) {
				setMilestoneBuyerDetails(res.data.data);
				return res.data.data;
			}
		} catch (error) {
			toast.error(
				error?.response?.data?.message ||
					"Failed to fetch buyer payment details",
			);
		} finally {
			setLoading(false);
		}
	...
	const exportMilestoneBuyers = async (params) => {
		try {
			setLoading(true);

			const res = await projectApi.exportMilestoneBuyers(params);
			...
			link.setAttribute(
				"download",
				`milestone-buyers-${Date.now()}.xlsx`,
			);
			...
			toast.error(
				error?.response?.data?.message ||
					"Failed to export milestone buyers",
			);
		} finally {
			setLoading(false);
		}
	...
	return {
		...
		milestoneBuyers,
		milestoneBuyerDetails,
		milestoneBuyersPagination,

		// Core
		fetchProject,
		createProject, // ✅ NEW - Create with coverImageKey & galleryImageKeys
		updateProject, // ✅ NEW - Update with coverImageKey & galleryImageKeys

		fetchMilestonePaymentSummary,
		...
		// Bookings
		cancelAllBookings, // ✅ NEW - Cancel all bookings
