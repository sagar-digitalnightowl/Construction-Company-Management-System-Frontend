...
export const useBooking = () => {
	...
	const fetchProjectInstallments = useCallback(
		async (projectId, params = {}) => {
			setLoading(true);
			try {
				const res = await bookingApi.getProjectInstallments(
					projectId,
					params,
				);
				setInstallments(res.data?.data?.installments || []);
	...
	const createInstallmentsFromTemplate = async (bookingId, data) => {
		setLoading(true);
		try {
			const res = await bookingApi.createInstallmentsFromTemplate(
				bookingId,
				data,
			);
	...
	const createCustomInstallments = async (bookingId, data) => {
		setLoading(true);
		try {
			const res = await bookingApi.createCustomInstallments(
				bookingId,
				data,
			);
	...
	const uploadAgreement = async (bookingId, file) => {
		try {
			...
			const presignRes = await bookingApi.getAgreementPresignedUrl(
				bookingId,
				{
					fileName: file.name,
					mimeType: file.type,
			...
			if (uploadUrl) {
				...
				await bookingApi.confirmAgreementUpload(bookingId, {
					fileKey,
