...
export const useHR = () => {
	...
	const [currentEmployeeSalarySlips, setCurrentEmployeeSalarySlips] =
		useState([]);
	...
	const [employeeWalletTransactions, setEmployeeWalletTransactions] =
		useState([]);
	...
	const [financeDashboardPagination, setFinanceDashboardPagination] =
		useState({
			page: 1,
			limit: 10,
			total: 0,
	...
	const [financeEmployeeExpenses, setFinanceEmployeeExpenses] =
		useState(null);
	const [financeExpenseSummary, setFinanceExpenseSummary] = useState(null);
	const [
		financeExpenseSummaryPagination,
		setFinanceExpenseSummaryPagination,
	] = useState({
		page: 1,
		limit: 10,
		total: 0,
	...
	const fetchEmployeeDashboard = useCallback(async () => {
		...
		try {
			const res = await hrApi.getEmployeeDashboard();
	...
	const generateBulkSalarySlips = async (data) => {
		...
		try {
			...
			toast.success(
				res.data?.message || "Bulk salary slips generated successfully",
			);
			return res.data?.data;
		} catch (err) {
			toast.error(
				err.response?.data?.message ||
					"Failed to generate bulk salary slips",
			);
			return null;
		} finally {
			setLoading(false);
	...
	const downloadSalaryReport = async (params) => {
		...
		try {
			...
			link.setAttribute(
				"download",
				`Salary_Report_${params.month}_${params.year}.xlsx`,
			);
	...
	const submitPayrollForApproval = async (data) => {
		...
		try {
			...
			toast.error(
				err.response?.data?.message || "Failed to submit payroll batch",
			);
			return false;
		} finally {
			setLoading(false);
	...
	const fetchEmployeeCurrentShift = useCallback(async (employeeId) => {
		...
		try {
			...
			const message =
				err.response?.data?.message || "Failed to load employee shift";
			...
			toast.error(message, { id: "employee-shift-error" });
	...
	const fetchEmployeeWalletTransactions = useCallback(
		async (employeeId, params = {}) => {
			setLoading(true);
			try {
				const res = await hrApi.getEmployeeWalletTransactions(
					employeeId,
					params,
				);
	...
	const createExpense = async (data) => {
		...
		const amount =
			data instanceof FormData ? data.get("amount") : data.amount;
		const categoryId =
			data instanceof FormData ? data.get("categoryId") : data.categoryId;
		const projectId =
			data instanceof FormData ? data.get("projectId") : data.projectId;
	...
	const refundExpenseTicket = async (id, data = {}) => {
		...
		try {
			...
			toast.error(
				err.response?.data?.message || "Failed to refund ticket",
			);
			return false;
		} finally {
			setLoading(false);
	...
	const createFinanceExpense = async (data) => {
		...
		const paidToUserId =
			data instanceof FormData
				? data.get("paidToUserId")
				: data.paidToUserId;
		const paidToName =
			data instanceof FormData ? data.get("paidToName") : data.paidToName;
		...
		const hasManualName =
			paidToName &&
			typeof paidToName === "string" &&
			paidToName.trim() !== "";

		if (!hasUserId && !hasManualName) {
			toast.error(
				"Payment Recipient is required (Select User or Enter Manually)",
			);
			return false;
		}
		...
		try {
			...
			toast.success(
				"Finance expense created successfully! Awaiting Admin approval.",
			);
	...
	const fetchFinanceEmployeeExpenses = useCallback(
		async (employeeId, params = {}) => {
			setLoading(true);
			try {
				const res = await hrApi.getFinanceEmployeeExpenses(
					employeeId,
					params,
				);
	...
	return {
		...
		financeDashboard,
		financeDashboardPagination,
		financeProjectExpenses,
		financeEmployeeExpenses,
		...
		fetchFinanceProjectExpenses,
		fetchFinanceEmployeeExpenses,
		fetchFinanceExpenseSummary,
	};
};
