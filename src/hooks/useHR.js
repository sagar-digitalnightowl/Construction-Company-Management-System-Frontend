...
//     // ─── PRESIGNED URL UPLOAD HELPERS (UPDATED) ───
//     const getPresignedUrl = async (file, fileType) => {
//         try {
//             const res = await hrApi.getPresignedUrl({
//                 fileName: file.name,
...
//             // Check for correct keys based on the API response structure ('url' and 'key')
...
//             // Return 'key' instead of 'publicUrl' since backend expects proofKey
...
export const useHR = () => {
    const [employees, setEmployees] = useState({ employees: [], pagination: { page: 1, limit: 10, total: 0, pages: 0 } });
    const [employee, setEmployee] = useState(null);
    const [employeeStats, setEmployeeStats] = useState(null);
    const [currentEmployeeAttendance, setCurrentEmployeeAttendance] = useState([]);
    const [currentEmployeeLeaves, setCurrentEmployeeLeaves] = useState([]);
    ...
    // Expenses State
    const [myExpenses, setMyExpenses] = useState([]);
    const [myExpensesPagination, setMyExpensesPagination] = useState({ page: 1, limit: 10, total: 0, pages: 0 });
    const [allExpenses, setAllExpenses] = useState([]);
    const [allExpensesPagination, setAllExpensesPagination] = useState({ page: 1, limit: 10, total: 0, pages: 0 });
    const [expenseDetail, setExpenseDetail] = useState(null);
    ...
    // ─── PRESIGNED URL UPLOAD HELPERS (UNTOUCHED / ORIGINAL) ───
    const getPresignedUrl = async (file, fileType) => {
        try {
            const res = await hrApi.getPresignedUrl({
                fileName: file.name,
    ...
    // ✅ EXACTLY HERE: FormData parsing for validation before sending to API
    const createExpense = async (data) => {
        ...
        const proof = data instanceof FormData ? data.get("proof") : (data.proof || data.proofKey);
        ...
        if (!proof) {
            toast.error("Proof file is required.");
            return false;
        }

        setLoading(true);
        try {
            await hrApi.createExpense(data); // Sending direct payload/FormData
            toast.success("Expense ticket raised successfully!");
            await fetchMyExpenses();
            return true;
        } catch (err) {
    ...
    return {
        ...
        shifts, employeeShift, labors, labor, laborStats, laborWages, laborAttendance, laborAttendanceSummary,
        announcements, myExpenses, myExpensesPagination, allExpenses, allExpensesPagination, expenseDetail,
        
        fetchMyExpenses, createExpense, fetchAllExpenses, fetchExpenseById, approveExpense, rejectExpense, payExpense,
        loading, pagination,
        currentEmployeeAttendance, currentEmployeeLeaves, currentEmployeeSalarySlips, currentEmployeeLeaveBalance,
        
        fetchEmployees, fetchEmployeeById, fetchEmployeeStats, createEmployee, uploadFile, registerEmployee, verifyOtp, updateEmployee, deleteEmployee, deleteUser,
        fetchCurrentEmployeeAttendance, fetchCurrentEmployeeLeaves, fetchCurrentEmployeeLeaveBalance, fetchCurrentEmployeeSalarySlips,
        ...
        fetchAllSalarySlips, fetchMySalarySlips, fetchEmployeeSalarySlips, generateSalarySlip, generateBulkSalarySlips, updateSalaryStatus, downloadSalaryReport, downloadSalarySlipPdf, submitPayrollForApproval, fetchMyPayrollBatches,
        fetchShifts, createShift, updateShift, deleteShift, assignShiftToEmployee, fetchEmployeeCurrentShift,
        fetchLabors, fetchLaborById, fetchLaborStats, createLabor, updateLabor, deleteLabor, fetchLaborWages, createLaborWage, updateLaborWage, deleteLaborWage, fetchLaborAttendance, markLaborAttendance, markBulkLaborAttendance, fetchLaborAttendanceSummary, fetchDailyLaborAttendance, getPresignedUrl, confirmUpload,
