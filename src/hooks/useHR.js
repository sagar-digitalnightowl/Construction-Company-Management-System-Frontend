...
//             if (params.month && params.year && !params.month.includes('-')) {
//                 const monthNames = [
//                     "January", "February", "March", "April", "May", "June",
//                     "July", "August", "September", "October", "November", "December"
//                 ];
//                 const mIndex = monthNames.indexOf(params.month);
//                 if (mIndex !== -1) {
//                     queryParams.month = `${params.year}-${String(mIndex + 1).padStart(2, '0')}`;
//                     delete queryParams.year;
//                 }
//             } else if (params.month && params.year && params.month.includes('-')) {
//                 delete queryParams.year;
//             }

//             const res = await hrApi.getAllSalarySlips(queryParams);
            
//             // Robust Array Extraction
//             let extractedSlips = [];
//             if (Array.isArray(res.data?.data)) {
//                 extractedSlips = res.data.data;
//             } else if (Array.isArray(res.data)) {
//                 extractedSlips = res.data;
//             } else if (res.data?.data?.salarySlips) {
//                 extractedSlips = res.data.data.salarySlips;
//             } else if (Array.isArray(res)) {
//                 extractedSlips = res;
//             }
            
//             setCompanySalarySlips(extractedSlips);
//             return res.data?.data;
//         } catch (err) {
//             toast.error("Failed to load company salary slips");
//             return null;
...
//             setEmployeeSalarySlips(extractedSlips);
//             return res.data?.data;
//         } catch (err) {
//             toast.error("Failed to load employee salary slips");
//             return null;
...
//         const amount = data instanceof FormData ? data.get("amount") : data.amount;
//         const categoryId = data instanceof FormData ? data.get("categoryId") : data.categoryId;
//         const projectId = data instanceof FormData ? data.get("projectId") : data.projectId; 
...
export const useHR = () => {
    ...
    // ==================== 🏦 NEW: Finance Expenses State ====================
    const [financeDashboard, setFinanceDashboard] = useState([]);
    const [financeDashboardPagination, setFinanceDashboardPagination] = useState({
        page: 1, limit: 10, total: 0, pages: 0,
    });
    const [financeProjectExpenses, setFinanceProjectExpenses] = useState(null);
    const [financeEmployeeExpenses, setFinanceEmployeeExpenses] = useState(null);
    const [financeExpenseSummary, setFinanceExpenseSummary] = useState(null);
    const [financeExpenseSummaryPagination, setFinanceExpenseSummaryPagination] = useState({
        page: 1, limit: 10, total: 0, pages: 0,
    });
    ...
    // ==================== 🏦 NEW: FINANCE EXPENSES & REPORTS ====================
    
   const createFinanceExpense = async (data) => {
        const title = data instanceof FormData ? data.get("title") : data.title;
        const amount = data instanceof FormData ? data.get("amount") : data.amount;
        const projectId = data instanceof FormData ? data.get("projectId") : data.projectId;
        
        // ✅ Dono fields ko extract karein
        const paidToUserId = data instanceof FormData ? data.get("paidToUserId") : data.paidToUserId;
        const paidToName = data instanceof FormData ? data.get("paidToName") : data.paidToName;
        ...
        // ✅ NAYA LOGIC: Ya toh valid User ID ho, ya fir Manual Name ho
        const hasUserId = paidToUserId && paidToUserId !== "none";
        const hasManualName = paidToName && (typeof paidToName === "string" && paidToName.trim() !== "");

        if (!hasUserId && !hasManualName) {
            toast.error("Payment Recipient is required (Select User or Enter Manually)");
            return false;
        }

        setLoading(true);
        try {
            await hrApi.createFinanceExpense(data);
            toast.success("Finance expense created successfully! Awaiting Admin approval.");
            await fetchFinanceDashboard();
            return true;
        } catch (err) {
            toast.error(err.response?.data?.message || "Failed to create finance expense");
            return false;
        } finally {
            setLoading(false);
        }
    ...
    const fetchFinanceDashboard = useCallback(async (params = {}) => {
        setLoading(true);
        try {
            const res = await hrApi.getFinanceExecutiveDashboard(params);
            const data = res.data?.data || [];
            // Backend returns array of tickets in `data` or `data.tickets` depending on exact response format
            setFinanceDashboard(Array.isArray(data) ? data : (data.tickets || []));
            if (res.data?.pagination) {
                setFinanceDashboardPagination(res.data.pagination);
            }
            return res.data;
        } catch (err) {
    ...
    const fetchFinanceProjectExpenses = useCallback(async (projectId) => {
        setLoading(true);
        try {
            const res = await hrApi.getFinanceProjectExpenses(projectId);
            setFinanceProjectExpenses(res.data?.data);
            return res.data?.data;
        } catch (err) {
            toast.error(err.response?.data?.message || "Failed to load project expenses");
            setFinanceProjectExpenses(null);
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const fetchFinanceEmployeeExpenses = useCallback(async (employeeId, params = {}) => {
        setLoading(true);
        try {
            const res = await hrApi.getFinanceEmployeeExpenses(employeeId, params);
            setFinanceEmployeeExpenses(res.data?.data);
            return res.data?.data;
        } catch (err) {
            toast.error(err.response?.data?.message || "Failed to load employee expenses");
            setFinanceEmployeeExpenses(null);
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const fetchFinanceExpenseSummary = useCallback(async (params = {}) => {
        setLoading(true);
        try {
            const res = await hrApi.getFinanceExpenseSummary(params);
            setFinanceExpenseSummary(res.data?.data);
            if (res.data?.pagination) {
                setFinanceExpenseSummaryPagination(res.data.pagination);
            }
            return res.data?.data;
        } catch (err) {
            toast.error(err.response?.data?.message || "Failed to load expense summary");
            setFinanceExpenseSummary(null);
            return null;
        } finally {
            setLoading(false);
        }
    ...
    return {
        ...
        financeDashboard, financeDashboardPagination, financeProjectExpenses,
        financeEmployeeExpenses, financeExpenseSummary, financeExpenseSummaryPagination,
        ...
        // 🏦 Finance Expense Functions
        createFinanceExpense, fetchFinanceDashboard, fetchFinanceProjectExpenses,
        fetchFinanceEmployeeExpenses, fetchFinanceExpenseSummary,
    };
};