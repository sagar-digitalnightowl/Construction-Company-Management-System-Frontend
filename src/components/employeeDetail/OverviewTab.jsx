			...
			<Card>
				...
				<CardContent className="space-y-3 text-sm">
					<div className="grid grid-cols-2 gap-2">
						<div>
							<span className="font-medium">PF Applicable:</span>{" "}
							<Badge variant={employee.jobDetails?.isPfApplicable ? "success" : "outline"}>
								{employee.jobDetails?.isPfApplicable ? "Yes" : "No"}
							</Badge>
						</div>
						<div>
							<span className="font-medium">ESI Applicable:</span>{" "}
							<Badge variant={employee.jobDetails?.isEsiApplicable ? "success" : "outline"}>
								{employee.jobDetails?.isEsiApplicable ? "Yes" : "No"}
							</Badge>
						</div>
					</div>
					...
					{employee.jobDetails?.isPfApplicable && (
						<div className="pt-2 border-t">
							<p className="font-medium mb-1">PF Details</p>
							<div className="grid grid-cols-2 gap-1 pl-2">
								<div>PF Number: {employee.jobDetails?.pfNumber || "—"}</div>
								<div>UAN Number: {employee.jobDetails?.uanNumber || "—"}</div>
								<div>
									Contribution:{" "}
								...
								<div>
									Joining Date:{" "}
									{employee.jobDetails?.pfJoiningDate
										? formatDate(employee.jobDetails.pfJoiningDate)
										: "—"}
								</div>
							</div>
						</div>
					...
					{employee.jobDetails?.isEsiApplicable && (
						<div className="pt-2 border-t">
							<p className="font-medium mb-1">ESI Details</p>
							<div className="grid grid-cols-2 gap-1 pl-2">
								<div>ESI Number: {employee.jobDetails?.esiNumber || "—"}</div>
								<div>
									Contribution:{" "}
								...
								<div>
									Joining Date:{" "}
									{employee.jobDetails?.esiJoiningDate
										? formatDate(employee.jobDetails.esiJoiningDate)
										: "—"}
								</div>
							</div>
						</div>
