...
export function BookingFormDialog({
	...
	useEffect(() => {
		const fetchFullBookingDetails = async () => {
			...
			try {
				...
				let activeFlatId = flatId;
				let flatObj = floorObj?.flats?.find((f) => f._id === flatId);

				if (!flatObj && fullBooking.flatSnapshot?.flatNumber) {
					flatObj = floorObj?.flats?.find(
						(f) => String(f.flatNumber) === String(fullBooking.flatSnapshot.flatNumber)
					);
					if (flatObj) {
						activeFlatId = flatObj._id;
					}
				}
				...
				setForm({
					...
					customerType: fullBooking.customerType || "",
	...
	const handleSubmit = async () => {
		...
		if (isEdit) {
			payload.projectManager = form.projectManager || null;
			payload.businessManager = form.businessManager || null;
			payload.teamManager = form.teamManager || null;

			payload.flatId = form.flatId;
		} else {
			payload.projectManager = form.projectManager || undefined;
			payload.businessManager = form.businessManager || undefined;
			payload.teamManager = form.teamManager || undefined;
			...
			<DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
				...
				<div className={`space-y-6 p-1 transition-opacity ${fetchingDetails ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						{/* ✅ Cleaned up Payment Modes - Added Demand Draft */}
						<div>
							<Label>Payment Mode</Label>
							<Select value={form.paymentMode} disabled={isEdit} onValueChange={(v) => updateForm("paymentMode", v)}>
								<SelectTrigger><SelectValue placeholder="Select payment mode" /></SelectTrigger>
								<SelectContent>
									{["Cash", "Cheque", "Bank Transfer", "Card", "NEFT", "RTGS", "Demand Draft"].map((mode) => (
						...
						<div>
							...
							<Select
								...
								onValueChange={(v) => updateForm("customerType", v)}
							>
								<SelectTrigger>
						...
						<div>
							...
							<Select
								value={form.projectManager || "none"}
								onValueChange={(value) =>
									updateForm(
										"projectManager",
										value === "none" ? "" : value
