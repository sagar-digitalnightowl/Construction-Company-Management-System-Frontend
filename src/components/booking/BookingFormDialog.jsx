            ...
            <div className="md:col-span-2">
              ...
              {selectedFlat && parseFloat(form.bookingAmount) > 0 && (
                <div className="mt-2 p-2 bg-muted/30 rounded-md text-xs flex gap-6 flex-wrap">
                  <span>
                    Booking Amount: <strong>{formatINR(parseFloat(form.bookingAmount) || 0)}</strong>
                  </span>
                  <span>
                    + GST ({getGSTPercentage()}%): <strong>{formatINR(getGSTOnBooking())}</strong>
                  </span>
                  <span className="font-semibold text-primary">
                    = Total: {formatINR(getTotalPayable())}
                  </span>
                          ...
                          {diff !== 0 && (
                            <div
                              className={`mt-1 font-medium ${
                                diff > 0 ? "text-amber-600" : "text-destructive"
                              }`}
                            >
                              {diff > 0
                                ? `⚠️ You need to add ${formatINR(diff)} more to match the target.`
