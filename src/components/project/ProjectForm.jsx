  ...
  return (
    <div className="col-span-2 border-t pt-4 mt-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Project Video */}
        <div>
          <Label className="text-xs mb-1 block">Project Video</Label>
          <div className="flex flex-wrap gap-3 mt-1">
            {/* Existing Uploaded Videos */}
            {existingVideos.map((vidUrl, idx) => (
              <div key={`existing-vid-${idx}`} className="relative w-24 h-20 rounded border bg-black overflow-hidden">
            ...
            {/* Upload Video Button - Only show if no video exists */}
            {existingVideos.length === 0 && newVideoPreviews.length === 0 && (
              <label className="cursor-pointer border-2 border-dashed rounded p-3 text-center hover:bg-muted/50 transition w-24 h-20 flex flex-col items-center justify-center">
                <Plus className="h-5 w-5 text-muted-foreground" />
                <span className="text-[10px] text-muted-foreground mt-1 text-center leading-tight">Add Video</span>
                <input type="file" accept="video/*" className="hidden" onChange={onNewVideoSelect} />
              </label>
            )}
          </div>
        </div>
...
export function ProjectForm({ open, onOpenChange, initialData, onSave }) {
  ...
  useEffect(() => {
    if (open) {
      if (initialData) {
        ...
        setCoverPreview(initialData.coverImage || null);
        setCoverKey(extractKey(initialData.coverImage)); 
        
        // Load existing gallery images (URLs for preview, Keys for Payload)
        setExistingGalleryImages(initialData.galleryImages || []);
        setExistingGalleryKeys((initialData.galleryImages || []).map(extractKey)); 

        // Load existing video 
        const videoUrl = initialData.projectVideo;
        setExistingVideos(videoUrl ? [videoUrl] : []);
        setExistingVideoKeys(videoUrl ? [extractKey(videoUrl)] : []);
      } else {
        setForm({ ...EMPTY_PROJECT, towers: [] });
        setCoverPreview(null);
        setCoverKey("");
  ...
  const handleNewVideoSelect = (e) => {
    ...
    // Replace array with just this one file
    setNewVideoFiles([file]);
    setNewVideoPreviews([URL.createObjectURL(file)]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let finalCoverKey = coverKey;
      let finalGalleryKeys = [...existingGalleryKeys];
      let finalVideoKey = existingVideoKeys.length > 0 ? existingVideoKeys[0] : undefined; 
      ...
      // 3. Upload Video
      if (newVideoFiles.length > 0) {
        finalVideoKey = await uploadMediaToCloud(newVideoFiles[0], "project");
      }
